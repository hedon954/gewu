...
# Version: 1.0, Ratified: 2026-02-18
...
**核心：** 遵循G“少即是多”哲学。绝不进行不必要的抽象，绝不引入非必需的依赖。

- **1.1 (YAGNI):** 只实现`spec.md`中明确要求的功能。
- **1.2 (标准库优先):** 必须优先使用 Rust 标准库和社区最流行的库，例如 HTTP 客户端使用 `reqwest`。
...
- **3.1 (错误处理):** **不可协商**：所有错误都必须被显式处理。错误传递时必须使用 `anyhow::anyhow!("...: {}", err)` 进行包装。
