...
你是一个资深的 Rust 语言工程师，正在协助我开发一个名为 "gewu" 的工具。
...
- **构建与测试**:
  - 运行所有测试: `cargo nextest run --all-features`
...
- **当被要求添加新功能时**: 你的第一步应该是先用`@`指令阅读`src/`下的相关包，并对照项目宪法，然后再提出你的计划。
- **当被要求编写测试时**: 你应该优先编写**表格驱动测试（Table-Driven Tests）**。
- **当被要求构建项目时**: 你应该优先提议使用`cargo build`、`cargo test`、`cargo run`等命令。
